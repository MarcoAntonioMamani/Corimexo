...
#Region "Metodos Privados"
    ...
    Private Sub _prCrearCarpetaImagenes()
        Dim rutaDestino As String = RutaGlobal + "\Imagenes\Imagenes CategoriaAlmacen\"

        If System.IO.Directory.Exists(RutaGlobal + "\Imagenes\Imagenes CategoriaAlmacen\") = False Then
            If System.IO.Directory.Exists(RutaGlobal + "\Imagenes") = False Then
                System.IO.Directory.CreateDirectory(RutaGlobal + "\Imagenes")
                If System.IO.Directory.Exists(RutaGlobal + "\Imagenes\Imagenes CategoriaAlmacen") = False Then
                    System.IO.Directory.CreateDirectory(RutaGlobal + "\Imagenes\Imagenes CategoriaAlmacen")
                End If
            Else
                If System.IO.Directory.Exists(RutaGlobal + "\Imagenes\Imagenes CategoriaAlmacen") = False Then
                    System.IO.Directory.CreateDirectory(RutaGlobal + "\Imagenes\Imagenes CategoriaAlmacen")
...
#Region "METODOS SOBRECARGADOS"
    ...
    Public Overrides Function _PMOGrabarRegistro() As Boolean
        ...
        Dim res As Boolean = L_fnGrabarMateriales(tbCodigoOriginal.Text, tbNombre.Text, tbdesccorta.Text, cbCategoria.Value, nameImg, IIf(swEstado.Value = True, 1, 0))


        If res Then
            Modificado = False
            _fnMoverImagenRuta(RutaGlobal + "\Imagenes\Imagenes CategoriaAlmacen", nameImg)
            nameImg = "Default.jpg"

            Dim img As Bitmap = New Bitmap(My.Resources.checked, 50, 50)
            ToastNotification.Show(Me, "Código de CategoriaAlmacen ".ToUpper + tbCodigoOriginal.Text + " Grabado con Exito.".ToUpper,
                                      img, 2000,
    ...
    Public Overrides Function _PMOModificarRegistro() As Boolean
        ...
        Dim nameImage As String = IIf(IsDBNull(JGrM_Buscador.GetValue("yhimg")), "", JGrM_Buscador.GetValue("yhimg"))
        If (Modificado = False And nameImage <> "") Then

            res = L_fnModificarMateriales(tbCodigoOriginal.Text, tbNombre.Text, tbdesccorta.Text, cbCategoria.Value, nameImage, IIf(swEstado.Value = True, 1, 0))

        Else

            res = L_fnModificarMateriales(tbCodigoOriginal.Text, tbNombre.Text, tbdesccorta.Text, cbCategoria.Value, nameImg, IIf(swEstado.Value = True, 1, 0))
    ...
    Public Sub _PrEliminarImage(nameimg As String)
        ...
        If (Not (_fnActionNuevo()) And (File.Exists(RutaGlobal + "\Imagenes\Imagenes CategoriaAlmacen" + nameimg))) Then
            UsImg.pbImage.Image.Dispose()
            UsImg.pbImage.Image = Nothing
            Try
                My.Computer.FileSystem.DeleteFile(RutaGlobal + "\Imagenes\Imagenes CategoriaAlmacen" + nameimg)
            Catch ex As Exception
    ...
    Public Overrides Sub _PMOEliminarRegistro()
        ...
        If (bandera = True) Then
            ...
            Dim res As Boolean = L_fnEliminarCategoriaProductoMateria(tbCodigoOriginal.Text, mensajeError)
            If res Then
                _PrEliminarImage(JGrM_Buscador.GetValue("yhimg"))
    ...
    Public Overrides Function _PMOGetTablaBuscador() As DataTable
        Dim dtBuscador As DataTable = L_fnGeneralCategoriaMateria()
        Return dtBuscador
    End Function

    Public Overrides Function _PMOGetListEstructuraBuscador() As List(Of Modelo.Celda)
        Dim listEstCeldas As New List(Of Modelo.Celda)
        'a.yhnumi ,a.yhnombre ,a.yhcodigo ,a.yhcategoria, libreria .ycdes3 as libreria ,a.yhimg ,a.yhestado ,a.yhfact ,a.yhhact ,a.yhuact

        listEstCeldas.Add(New Modelo.Celda("yhnumi", True, "Código".ToUpper, 80))
        listEstCeldas.Add(New Modelo.Celda("yhnombre", True, "nombre".ToUpper, 200))
        listEstCeldas.Add(New Modelo.Celda("yhcodigo", True, "Codificacion".ToUpper, 250))
        listEstCeldas.Add(New Modelo.Celda("libreria", True, "Categoria".ToUpper, 120))
        listEstCeldas.Add(New Modelo.Celda("yhcategoria", False))
        listEstCeldas.Add(New Modelo.Celda("yhimg", False))
        listEstCeldas.Add(New Modelo.Celda("yhestado", True, "Estado".ToUpper, 90))
        listEstCeldas.Add(New Modelo.Celda("yhfact", False))
        listEstCeldas.Add(New Modelo.Celda("yhhact", False))
        listEstCeldas.Add(New Modelo.Celda("yhuact", False))
    ...
    Public Overrides Sub _PMOMostrarRegistro(_N As Integer)
        ...
        Try
            tbCodigoOriginal.Text = JGrM_Buscador.GetValue("yhnumi").ToString
        Catch ex As Exception
            Exit Sub
        End Try
        With JGrM_Buscador
            tbCodigoOriginal.Text = .GetValue("yhnumi").ToString
            tbNombre.Text = .GetValue("yhnombre")
            tbdesccorta.Text = .GetValue("yhcodigo")
            cbCategoria.Value = .GetValue("yhcategoria")
            lbFecha.Text = CType(.GetValue("yhfact"), Date).ToString("dd/MM/yyyy")
            lbHora.Text = .GetValue("yhhact").ToString
            lbUsuario.Text = .GetValue("yhuact").ToString

        End With
        Dim name As String = IIf(IsDBNull(JGrM_Buscador.GetValue("yhimg")), "", JGrM_Buscador.GetValue("yhimg"))
        If name.Equals("Default.jpg") Or Not File.Exists(RutaGlobal + "\Imagenes\Imagenes CategoriaAlmacen" + name) Then

            Dim im As New Bitmap(My.Resources.pantalla)
            UsImg.pbImage.Image = im
        Else
            If (File.Exists(RutaGlobal + "\Imagenes\Imagenes CategoriaAlmacen" + name)) Then
                Dim Bin As New MemoryStream
                Dim im As New Bitmap(New Bitmap(RutaGlobal + "\Imagenes\Imagenes CategoriaAlmacen" + name))
